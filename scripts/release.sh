...
echo "     ⚠️ 预发布必须用 -a 写注释，CI 拿注释正文当 Release 说明："
echo "        git tag -a v<版本号> -m \"\$(printf 'WaterReminder <版本号>\\n\\n1. xxx\\n2. yyy\\n')\""
echo "        不写注释（lightweight tag）不会报错，但 Release 说明会退化成"
echo "        「本次发版没有填写更新说明」，且事后补不上，只能重打 tag。"
